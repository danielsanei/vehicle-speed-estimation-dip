...
def get_dark_channel(image, patch_size=15):
    return cv2.erode(np.min(image, axis=2), np.ones(patch_size, np.uint8), iterations=1)
...
def estimate_transmission(image, atmospheric_light, omega=1, patch_size=15):
...
def recover_image(image, atmospheric_light, transmission, eps=0.001):
    ...
    recovered_image = np.clip((image - atmospheric_light) / transmission[:, :, np.newaxis] + atmospheric_light, 0, 255).astype(np.uint8)
...
def dehaze(frame):
    ...
    recovered_image = recover_image(frame, atmospheric_light, transmission_map)
    return recovered_image

def demosaic(frame):
    demosaiced_image = colour_demosaicing.demosaicing_CFA_Bayer_Malvar2004(frame, pattern='RGGB')
...
if __name__ == "__main__":
    ...
    while True:
        ...
        [dehazed_frame, transmission_map] = remove_haze(frame)
        frame_count += 1
